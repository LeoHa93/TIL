...
SET BITS=64
SET USER_JDK64_FILE=%USERPROFILE%\.AndroidStudio4.0\config\studio%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%USERPROFILE%\.AndroidStudio4.0\config\studio%BITS%.exe.jdk
...
IF NOT EXIST "%VM_OPTIONS_FILE%" (
  :: user-overridden
  SET VM_OPTIONS_FILE=%USERPROFILE%\.AndroidStudio4.0\config\studio%BITS%.exe.vmoptions
)
IF NOT EXIST "%VM_OPTIONS_FILE%" (
...
SET COMMON_JVM_ARGS="-XX:ErrorFile=%USERPROFILE%\java_error_in_STUDIO_%%p.log" "-XX:HeapDumpPath=%USERPROFILE%\java_error_in_STUDIO.hprof" -Didea.paths.selector=AndroidStudio4.0 %IDE_PROPERTIES_PROPERTY%
